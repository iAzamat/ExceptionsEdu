...
import Homeworks.Java03.Exception.SexException;
import Homeworks.Java03.Modules.InputData;
        ...
        for (String i : data) {
            if (i.length() == 1) {
                ...
                if (Integer.parseInt(arrayDate[0]) < 0 ||
                        ...
                        Integer.parseInt(arrayDate[2]) > year) {
                    try {
                ...
                dataDic.put("tel", i);
            } else if (i.matches("[A-Za-z]+") || i.matches("[А-ЯЁа-яё]+")) {
                sb.append(i + " ");
            } else {
                try {
                    throw new DataException();
